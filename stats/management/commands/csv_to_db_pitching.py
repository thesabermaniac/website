...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for index, row in stats_df.iterrows():
            ...
            if player:
                pitching_stats, created = PitchingStatistics.objects.get_or_create(player=player, year=options['year'],
                                                                                 is_projection=options['is_proj'],
                                                                                 projection_system=options['proj_sys'])
                for col, value in row.items():
                    if col in pitching_stats.get_field_names():
                        setattr(pitching_stats, col, value)
                pitching_stats.save()
